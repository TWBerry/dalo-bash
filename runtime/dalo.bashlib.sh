...
# ============================================================================
# 23. STEP35 OBJECT DEFINITION ABI v1
# ============================================================================
#
...
#   - They are NOT copied wholesale into a generated MACHINE.
#   - They describe ports, required methods/features and execution constraints.
#   - jq is used only by the compiler/definition layer; generated runtime code
#     will not depend on jq merely because its type was described by JSON.

ASYNC_OBJECT_DEFINITION_ABI=1
declare -g -A ASYNC_OBJECT_DEF_FILE=()
declare -g -A ASYNC_OBJECT_DEF_JSON=()

async_object_definition_register() {
    [ $# -eq 1 ] || return 2
    local file="$1" type
    [[ -r "$file" ]] || return 3
    ...
    jq -e '
      ...
      ([.ports[] |
          (type == "object") and
          (.direction == "input" or .direction == "output") and
          (.plane == "data" or .plane == "control") and
          ((.type // "any") | type == "string")
       ] | all) and
      ((.methods // []) | type == "array" and all(.[]; type == "string" and length > 0)) and
      ((.features // []) | type == "array" and all(.[]; type == "string" and length > 0)) and
      ((.execution // []) | type == "array" and all(.[]; type == "string" and length > 0)) and
      ((.worker // {"required":false}) | type == "object" and
          ((.required // false) | type == "boolean"))
    ' "$file" >/dev/null || return 5

    type="$(jq -r '.type' "$file")" || return
    ASYNC_OBJECT_DEF_FILE["$type"]="$file"
    ASYNC_OBJECT_DEF_JSON["$type"]="$(cat -- "$file")"
}

async_object_definition_has() {
    [ $# -eq 1 ] || return 2
    [[ -n "${ASYNC_OBJECT_DEF_FILE[$1]:-}" ]]
}

async_object_definition_port() {
    [ $# -eq 3 ] || return 2
    local type="$1" port="$2" field="$3"
    local file="${ASYNC_OBJECT_DEF_FILE[$type]:-}"
    [[ -n "$file" ]] || return 3
    jq -er --arg p "$port" --arg f "$field" '
      .ports[$p] as $port |
      if $port == null then error("unknown port")
      elif ($port[$f] == null) then error("unknown port field")
      else $port[$f] end
    ' "$file"
}

async_object_definition_list() {
    [ $# -eq 2 ] || return 2
    local file="${ASYNC_OBJECT_DEF_FILE[$1]:-}"
    [[ -n "$file" ]] || return 3
    jq -er --arg f "$2" '.[$f] // [] | .[]' "$file"
}

async_object_definition_worker_required() {
    [ $# -eq 1 ] || return 2
    local file="${ASYNC_OBJECT_DEF_FILE[$1]:-}"
    [[ -n "$file" ]] || return 3
    jq -e '(.worker.required // false) == true' "$file" >/dev/null
}

async_object_definition_execution_allowed() {
    [ $# -eq 2 ] || return 2
    local file="${ASYNC_OBJECT_DEF_FILE[$1]:-}"
    [[ -n "$file" ]] || return 3
    jq -e --arg mode "$2" '(.execution // []) | index($mode) != null' "$file" >/dev/null
}

async_project_validate_definitions() {
    [ $# -eq 1 ] || return 2
    local project="$1"
    local -n objs="${project}_DECL_OBJECTS"
    local -n types="${project}_DECL_TYPE"
    local -n edges="${project}_EDGES"
    local obj type edge src sport dst dport sdir ddir splane dplane stype dtype

    for obj in "${objs[@]}"; do
        type="${types[$obj]:-}"
        async_object_definition_has "$type" || {
            printf 'PROJECT %s: undefined object type %s for %s\n' "$project" "$type" "$obj" >&2
            return 20
        }
    done

    for edge in "${edges[@]}"; do
        IFS=$'\t' read -r src sport dst dport <<<"$edge"

        type="${types[$src]:-}"
        sdir="$(async_object_definition_port "$type" "$sport" direction)" || return 21
        [[ "$sdir" == output ]] || return 22
        splane="$(async_object_definition_port "$type" "$sport" plane)" || return 23
        stype="$(async_object_definition_port "$type" "$sport" type)" || return 24

        type="${types[$dst]:-}"
        ddir="$(async_object_definition_port "$type" "$dport" direction)" || return 25
        [[ "$ddir" == input ]] || return 26
        dplane="$(async_object_definition_port "$type" "$dport" plane)" || return 27
        dtype="$(async_object_definition_port "$type" "$dport" type)" || return 28
